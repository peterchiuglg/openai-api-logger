...
def chat_completion(messages, max_tokens, temperature, top_p, frequency_penalty, model=MODEL_CHAT_COMPLETION,
                    timeout=60):
    t0 = time()
    response = client.chat.completions.create(model=model, temperature=temperature, max_tokens=max_tokens, top_p=top_p,
                                              frequency_penalty=frequency_penalty, messages=messages, timeout=timeout)
    time_taken = time() - t0
...
def embed_one(text, model=MODEL_EMBEDDING, return_usage=False):
    t0 = time()
...
def embed(texts, model=MODEL_EMBEDDING, batch=None, return_usage=False):
